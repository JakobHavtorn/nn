...
by = np.zeros((input_size))  # output bias
...
def lossFun(inputs, targets, hprev):
  ...
  for t in range(len(inputs)):
    ...
    # # no output layer (direct usage of hidden state)
    # ps[t] = np.exp(hs[t]) / np.sum(np.exp(hs[t]))
    # loss += -np.log(ps[t][targets[t]])  # softmax (cross-entropy loss)
  ...
  for t in reversed(range(len(inputs))):
    ...
    dy[targets[t]] -= 1
    dWhy += np.outer(dy, hs[t].T)
    dby += dy
    dh = np.dot(Why.T, dy) + dhnext  # backprop into h
    dhraw = (1 - hs[t] * hs[t]) * dh  # backprop through tanh nonlinearity
    dbh += dhraw
    dWxh += np.outer(dhraw, xs[t].T)
    dWhh += np.outer(dhraw, hs[t-1].T)
...
while True:
  ...
  if n > epoch_len or n == 0:
    hprev = np.zeros((hidden_size))  # reset RNN memory
    np.random.seed(0)

#   inputs = np.random.randn(seq_length, input_size)
  inputs = np.random.uniform(low=0, high=1, size=(seq_length, input_size))
  targets = np.round(np.stack([inputs[0]] * seq_length), decimals=0).astype(np.int32)
...
#     sample_ix = sample(hprev, inputs[0], 200)
  ...
  if n % 100 == 0:
    print('iter %d, loss: %f' % (n, smooth_loss.mean()))  # print progress
